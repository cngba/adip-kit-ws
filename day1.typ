...
Đặc điểm:
- Chụp với thời gian lặp dài (TR-Repetition) và thời gian vang dài (TE-Echo time).
...
2. Màn hình quan sát chính MRML (MRML Scene):

- Màn hình quan sát chính này tương tác trực tiếp với các mô-đun (Modules) khác nhau.
- Là nơi chứa và tổ chức tất cả các nút (Nodes), cũng như quản lý dữ liệu và mối quan hệ giữa chúng.

3. Các Mô-đun (Modules):
- Bao gồm nhiều mô-đun từ cơ bản (Segment Editor, Transforms, Registration) đến nâng cao (DICOM, Radiomics, SlicerIGT)
- Các mô-đun này thực hiện các chức năng chuyên biệt, ví dụ: phân đoạn, đăng ký hình ảnh hoặc phân tích.

Chúng tương tác với màn hình quan sát chính MRML để đọc và ghi dữ liệu.
...
4. Cách tạo một mô-đun tùy chỉnh:
- Chọn mục Module Selection >> Developer Tools >> Extension Wizard. 
- Chọn Create Extension. Trong một tiện ích mở rộng, có thể có nhiều mô-đun khác nhau. Đặt tên và phân loại của tiện ích.
- Thêm một mô-đun mới trong tiện ích.
- Cấu trúc của tiện ích có thể được quan sát trong Extension Editor. Tính chất của mỗi module có thể được chỉnh sửa bằng file .py trong tiện ích.
- Trên thanh Module Selection, chọn mô-đun đã tự cài đặt bằng cách chọn phân loại và tên của tiện ích.
5. Tiện ích mở rộng (Extensions):
- Bao gồm các mô-đun mở rộng như SlicerIGT, SlicerRT, SlicerIGSO.
- Các tiện ích này mở rộng thêm chức năng của chương trình, phù hợp với các nhu cầu cụ thể.

6. Thư viện tích hợp (Libraries):
...
Hình ảnh kỹ thuật số và truyền thông trong y học (Digital Imaging and Communications in Medicine - DICOM) là một tiêu chuẩn được sử dụng rộng rãi để trao đổi thông tin về X quang kỹ thuật số. Trong hầu hết các trường hợp, thiết bị hình ảnh (máy quét CT và MR) được sử dụng trong bệnh viện sẽ tạo ra hình ảnh được lưu dưới dạng đối tượng DICOM.

DICOM sắp xếp dữ liệu theo thứ bậc

- Bệnh nhân … có thể có 1 hoặc nhiều
    - Nghiên cứu (lần khám chụp ảnh đơn) … có thể có 1 hoặc nhiều
      - Chuỗi (thu thập hình ảnh đơn, thường tương ứng với một khối lượng hình ảnh đơn) … có thể có 1 hoặc nhiều
        - Trường hợp (thường là mỗi Chuỗi sẽ chứa nhiều Trường hợp, với mỗi Trường hợp tương ứng với một lát cắt duy nhất của hình ảnh)

Kết quả của quá trình chụp ảnh, thiết bị chụp ảnh sẽ tạo ra các tệp DICOM, trong đó mỗi tệp tương ứng với một Trường hợp và được gắn thẻ thông tin cho phép xác định thông tin về Loạt, Nghiên cứu và Bệnh nhân để đưa vào đúng vị trí trong hệ thống phân cấp.

Có nhiều đối tượng DICOM được định nghĩa theo tiêu chuẩn. Các loại đối tượng phổ biến nhất là các đối tượng lưu trữ khối lượng hình ảnh do máy quét CT và MR tạo ra. Những đối tượng đó thường có nhiều tệp (thể hiện) cho mỗi chuỗi. Các tác vụ xử lý hình ảnh thường liên quan đến việc phân tích toàn bộ khối lượng hình ảnh, thường tương ứng với một Chuỗi duy nhất.

*Cơ sở dữ liệu DICOM của Slicer*

Để sắp xếp dữ liệu và cho phép truy cập nhanh hơn, Slicer lưu trữ Cơ sở dữ liệu DICOM cục bộ chứa các bản sao (hoặc liên kết đến) tệp DICOM và thông tin cơ bản về nội dung của từng tệp. Có thể có nhiều cơ sở dữ liệu trên máy tính cùng một lúc và chuyển đổi giữa chúng nếu, ví dụ, chúng bao gồm dữ liệu từ các dự án nghiên cứu khác nhau. Mỗi cơ sở dữ liệu chỉ đơn giản là một thư mục trên đĩa cục bộ có một vài tệp SQLite và thư mục con để lưu trữ dữ liệu hình ảnh. Không sửa đổi thủ công nội dung của các thư mục này. Dữ liệu DICOM có thể nhập vào cơ sở dữ liệu thông qua nhập tệp hoặc thông qua truyền mạng DICOM. Các mô-đun Slicer cũng có thể điền kết quả tính toán vào cơ sở dữ liệu DICOM.


Lưu ý rằng tiêu chuẩn DICOM không chỉ định cách các tệp sẽ được sắp xếp trên đĩa, vì vậy nếu nguời dùng có dữ liệu DICOM từ CDROM hoặc được chuyển từ máy quét, nhìn chung không thể biết bất cứ điều gì về nội dung từ tên tệp hoặc thư mục. Tuy nhiên, sau khi dữ liệu được nhập vào cơ sở dữ liệu, nó sẽ được sắp xếp theo phân cấp Bệnh nhân/Nghiên cứu/Loạt tiêu chuẩn DICOM.

*Các plugin DICOM*

Một chức năng chính của mô-đun DICOM là ánh xạ từ tổ chức dữ liệu thu thập thành biểu diễn khối lượng. Nghĩa là, các tệp DICOM thường mô tả các thuộc tính của quá trình chụp ảnh, như trình tự các vị trí của bảng trong quá trình thu thập CT, trong khi Slicer hoạt động trên các khối lượng ảnh có các pixel cách đều nhau. Ví dụ, nếu tốc độ chuyển động của bàn không nhất quán trong quá trình thu thập (có thể xảy ra đối với một số lần quét 'bolus chase' tương phản), mô-đun DICOM của Slicer sẽ cảnh báo người dùng rằng hình học thu thập không nhất quán và người dùng nên thận trọng khi diễn giải kết quả phân tích như phép đo.

Điều này có nghĩa là Slicer thường có thể gợi ý nhiều cách diễn giải dữ liệu (chẳng hạn như đọc tệp DICOM dưới dạng tập dữ liệu khuếch tán hoặc dưới dạng thể tích vô hướng. Khi có thể tính toán bằng cách kiểm tra các tệp, mô-đun DICOM sẽ chọn tùy chọn diễn giải có khả năng xảy ra nhất theo mặc định. Kể từ bản phát hành này, các plugin tiêu chuẩn bao gồm thể tích vô hướng và thể tích khuếch tán, trong khi các tiện ích mở rộng có sẵn cho các đối tượng phân đoạn, dữ liệu RT và dữ liệu PET/CT. Dự kiến ​​sẽ có nhiều plugin hơn cho các phiên bản trong tương lai. Mục tiêu dài hạn là có thể biểu diễn hầu hết, nếu không muốn nói là toàn bộ, dữ liệu của Slicer trong các đối tượng dữ liệu DICOM tương ứng khi tiêu chuẩn phát triển để hỗ trợ chúng.

*Cách tạo cơ sở dữ liệu DICOM*
Tạo cơ sở dữ liệu DICOM là điều kiện tiên quyết cho tất cả các hoạt động DICOM. Khi mô-đun DICOM được mở lần đầu tiên, Slicer sẽ tự động tạo cơ sở dữ liệu mới. Người dùng có thể chọn tạo cơ sở dữ liệu mới hoặc mở cơ sở dữ liệu đã tạo trước đó.

Người dùng có thể mở cơ sở dữ liệu ở vị trí khác bất kỳ lúc nào trong bảng điều khiển mô-đun DICOM / Cài đặt cơ sở dữ liệu DICOM / Vị trí cơ sở dữ liệu.

*Đọc các tệp DICOM vào màn hình quan sát chính*
Vì các tệp DICOM thường nằm trong nhiều thư mục, chúng có thể tham chiếu chéo lẫn nhau và thường có thể được diễn giải theo nhiều cách khác nhau, nên việc đọc các tệp DICOM vào màn hình quan sát chính được thực hiện theo hai bước riêng biệt: nhập (lập chỉ mục các tệp để có thể hiển thị chúng trong trình duyệt cơ sở dữ liệu DICOM) và tải (hiển thị các mục DICOM đã chọn trong màn hình quan sát chính Slicer).

*Nhập DICOM*
- Đảm bảo rằng tất cả các tiện ích mở rộng Slicer bắt buộc đều được cài đặt. Lõi Slicer chứa plugin nhập DICOM để nhập hình ảnh, nhưng có thể cần thêm các tiện ích mở rộng khác cho các đối tượng thông tin khác. Ví dụ, tiện ích mở rộng SlicerRT là cần thiết để nhập/xuất các đối tượng thông tin về xạ trị (bộ cấu trúc RT, liều lượng, hình ảnh, kế hoạch). Tiện ích mở rộng báo cáo định lượng là cần thiết để nhập/xuất các đối tượng phân đoạn DICOM, báo cáo có cấu trúc và bản đồ tham số.

- Chọn mô-đun của DICOM

- Chọn các thư mục chứa các tệp của DICOM

- Kéo thư mục cứa các tệp DICOM vào cửa sổ chương trình Slicer hoặc chọn nút "Import" ở góc trên bên trái ở trình duyệt DICOM rồi chọn thư mục chứa các tệp DICOM.

Nút nhập có một menu thả xuống, có thể hiển thị bằng cách nhấp vào nút mũi tên xuống nhỏ ở bên phải. Hiện tại, mục duy nhất trong menu là tùy chọn “Sao chép tệp đã nhập vào cơ sở dữ liệu DICOM” (“Copy imported files to DICOM database”). Bật tùy chọn này để sao chép tất cả các tệp DICOM đã nhập từ vị trí gốc của chúng vào cơ sở dữ liệu DICOM của Slicer. Điều này hữu ích khi tải dữ liệu từ phương tiện lưu trữ có thể tháo rời (ổ đĩa CD/DVD/USB hoặc ổ đĩa từ xa), để có thể tải bộ dữ liệu ngay cả sau khi phương tiện đã được đẩy ra. Nếu tùy chọn sao chép bị tắt thì chỉ đường dẫn của các tệp đã nhập sẽ được lưu trữ trong cơ sở dữ liệu DICOM (cùng với các giá trị của các thẻ DICOM thường dùng nhất).

*DICOM loading*

Đi đến mô-đun DICOM. Nhấp vào “Show DICOM database” nếu cửa sổ cơ sở dữ liệu DICOM chưa hiển thị (nó hiển thị danh sách bệnh nhân, nghiên cứu và loạt).

Nhấp đúp vào bệnh nhân, nghiên cứu hoặc loạt để tải.

Nhấp vào nút “Show DICOM database” để chuyển đổi giữa trình duyệt cơ sở dữ liệu (để tải thêm dữ liệu) và trình xem (để xem những gì đã được tải vào màn hình quan sát chính)

*Xóa dữ liệu khỏi cơ sở dữ liệu DICOM*

Bằng cách nhấp chuột phải vào Bệnh nhân, Nghiên cứu hoặc Series, bạn có thể xóa mục nhập khỏi cơ sở dữ liệu DICOM. Lưu ý rằng để tránh mất dữ liệu vô tình, Slicer không xóa các tệp dữ liệu hình ảnh tương ứng nếu chỉ có liên kết của chúng được thêm vào cơ sở dữ liệu. Các tệp DICOM được sao chép vào cơ sở dữ liệu DICOM sẽ bị xóa khỏi cơ sở dữ liệu.


*Xuất dữ liệu từ màn hình quan sát chính đến cơ sở dữ liệu DICOM*
Dữ liệu trong màn hình quan sát chính có thể được xuất sang định dạng DICOM, để lưu trữ trong cơ sở dữ liệu DICOM hoặc xuất sang các tệp DICOM:

1. Đảm bảo rằng tất cả các tiện ích mở rộng Slicer bắt buộc đều được cài đặt. Lõi Slicer chứa plugin xuất DICOM để xuất hình ảnh, nhưng có thể cần thêm các tiện ích mở rộng khác cho các đối tượng thông tin khác.
- Cần có tiện ích mở rộng "SlicerRT" để nhập/xuất các đối tượng thông tin xạ trị: bộ cấu trúc RT, liều RT, hình ảnh RT, kế hoạch RT.
- Cần có tiện ích mở rộng "Quantitative reporting" để nhập/xuất các đối tượng phân đoạn DICOM, báo cáo có cấu trúc và bản đồ tham số.
2. Đi tới Mô-đun dữ liệu hoặc Mô-đun DICOM.
3. Nhấp chuột phải vào một nút dữ liệu trong cây dữ liệu sẽ được chuyển đổi sang định dạng DICOM.
4. Chọn loại xuất ở góc dưới bên trái của hộp thoại xuất. Điều này là cần thiết vì có thể có một số đối tượng thông tin DICOM có thể lưu trữ cùng một loại dữ liệu. Ví dụ: phân đoạn có thể được lưu trữ dưới dạng đối tượng phân đoạn DICOM (DICOM hiện đại) hoặc bộ cấu trúc RT (biểu diễn cũ, chủ yếu được sử dụng trong lập kế hoạch điều trị bức xạ).
- Loại xuất "Gói dữ liệu Slicer" ghi toàn bộ màn hình quan sát chính vào định dạng DICOM bằng cách đóng gói gói MRB cảnh bên trong tệp DICOM. Kết quả là đối tượng chụp thứ cấp DICOM, có thể được lưu trữ trong bất kỳ hệ thống lưu trữ DICOM nào. Thông tin chụp thứ cấp này lưu trữ tất cả các chi tiết của màn hình quan sát chính nhưng chỉ có 3D Slicer mới có thể diễn giải dữ liệu.
- Loại xuất: Sau khi người dùng chọn một nút, các plugin DICOM sẽ tạo các tệp có thể xuất cho chuỗi mà chúng có thể xuất. Danh sách kết quả sẽ xuất hiện trong phần này, được nhóm theo plugin. Số tin cậy sẽ là số tin cậy trung bình của từng chuỗi riêng biệt cho plugin đó.
5. Tùy chọn: Chỉnh sửa thẻ DICOM sẽ được sử dụng trong các tập dữ liệu đã xuất. Siêu dữ liệu từ nghiên cứu đã chọn sẽ được tự động điền vào hộp thoại Xuất và bạn có thể chọn một khối lượng Slicer để xuất.
- Trình chỉnh sửa thẻ DICOM bao gồm một danh sách các bảng. Bảng cho các thẻ chung cho bệnh nhân và nghiên cứu ở trên cùng và các thẻ cho từng loạt bên dưới chúng.
- "Thẻ" (Tags) trong bảng hiển thị không phải lúc nào cũng được ghi trực tiếp vào thẻ DICOM, chúng chỉ được các plugin DICOM sử dụng để điền thẻ DICOM vào các tệp đã xuất. Điều này cho phép linh hoạt hơn nhiều và các plugin DICOM có thể tự động điền một số thông tin và các plugin có thể hiển thị các tùy chọn xuất khác trong danh sách này (ví dụ: nén, quy ước đặt tên).
- Lưu các thẻ đã sửa đổi: đánh dấu vào hộp kiểm này để lưu các giá trị thẻ mới trong màn hình quan sát chính một cách liên tục.
- Cách thiết lập thẻ định danh duy nhất (Unique ID - UID):
  - Thẻ StudyInstanceUID chỉ định bệnh nhân và nghiên cứu nào sẽ được thêm vào chuỗi mới. Nếu giá trị được đặt thành trống thì một nghiên cứu mới sẽ được tạo. Nên giữ nguyên tất cả các giá trị bệnh nhân và nghiên cứu (PatientName, PatientID, StudyID, v.v.) trong các chuỗi trong cùng một nghiên cứu.
  - Thẻ SeriesInstanceUID xác định một chuỗi hình ảnh. Theo mặc định, giá trị của thẻ được đặt thành trống, điều này sẽ dẫn đến việc tạo một UID mới và do đó là một chuỗi mới. Trong những trường hợp rất hiếm, người dùng có thể muốn chỉ định một UID, nhưng UID không thể là bất kỳ UID hiện có nào vì điều đó sẽ dẫn đến việc các lát ảnh được xuất ra bị trộn lẫn vào một chuỗi khác. Do đó, UID chỉ được chấp nhận nếu nó không được sử dụng cho bất kỳ hình ảnh nào đã có trong cơ sở dữ liệu.
- Thẻ FrameOfReferenceUID chỉ định một tham chiếu không gian. Nếu hai hình ảnh có cùng giá trị UID khung tham chiếu thì điều đó có nghĩa là chúng được căn chỉnh không gian. Theo mặc định, giá trị là trống, nghĩa là một UID khung tham chiếu mới được tạo ra và do đó hình ảnh được xuất không được liên kết với bất kỳ hình ảnh nào khác. Nếu một hình ảnh được đăng ký không gian với một hình ảnh khác thì nên sao chép giá trị UID khung tham chiếu từ hình ảnh khác, vì điều này có thể cần thiết để hiển thị hợp nhất các hình ảnh trong một số phần mềm xem lại hình ảnh.
6. Nhấp vào Xuất
- Trong trường hợp có bất kỳ lỗi nào, một thông báo ngắn sẽ được hiển thị. Thông tin chi tiết hơn về lỗi được cung cấp trong nhật ký ứng dụng.
...
Các thành phần Slicer khác (Logic và GUI) quan sát các thay đổi trong màn hình quan sát chính MRML và các nút riêng lẻ và xử lý các sự kiện MRML thay đổi.
...
MRML Scene cung cấp cơ chế Hoàn tác/Làm lại để khôi phục trạng thái trước đó của màn hình quan sát chính và các nút riêng lẻ.
...
Sử dụng vtkSetReferenceStringMacro để đặt ID tham chiếu (nó đăng ký tham chiếu với màn hình quan sát chính).

Các phương thức truy cập sẽ kiểm tra xem nút được tham chiếu có vẫn nằm trong màn hình quan sát chính MRML bằng ID của nó hay không.
