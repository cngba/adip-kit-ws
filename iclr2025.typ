...
#let font-family = ("New Computer Modern", "Times", "Serif")

#let font-family-sans = ("New Computer Modern Sans", "Sans")

#let font-family-mono = ("New Computer Modern Mono", "Mono")
 ...
 */
 ...
 #let header-title = (
   ...
   [Báo cáo: Workshop Xử lý ảnh y khoa],  // accepted
)
...
#let iclr2025(
  ...
  show raw.where(block: true): it => { set par(justify: false); grid(
    ...
    block(width: 100%, inset: 1em, {
      ...
      for line in it.text.split("\n") {
        box(width: 0pt, align(right, str(i) + h(2em)))
        i = i + 1
        hide(line)
        linebreak()
      }
  ...
  show heading: it => {
    ...
    if level == 1 {
      align(center, text(size: font-size.large, weight: "bold", context {
      ...
      text(size: font-size.normal, weight: "bold", context {
        let ex = measure(smallcaps[x]).height
  ...
  set figure(numbering: "1", supplement: [Hình])
  ...
  if bibliography != none {
    set std-bibliography(title: [Tham khảo])
    bibliography
  }
